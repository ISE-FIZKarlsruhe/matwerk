...
def _normalize_state(raw: dict) -> dict:
    if not isinstance(raw, dict):
        return {"instances": {}, "nodes": {}, "file_graphs": {}, "file_ids": {}, "graphs": {}}
    ...
    if isinstance(rn, dict):
        if any(isinstance(v, dict) for v in rn.values()) or any('|' in k for k in rn.keys() if isinstance(k, str)):
            nodes = _flatten_legacy_nodes(rn)
        else:
            for k, v in rn.items():
                if isinstance(v, list):
                    nodes[str(k)] = [x for x in v if isinstance(x, str)]
    ...
    return {"instances": instances, "nodes": nodes, "file_graphs": file_graphs, "file_ids": file_ids, "graphs": graphs}
