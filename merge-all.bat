...
docker run --rm -v %cd%:/work -w /work -e "ROBOT_JAVA_ARGS=-Xmx8G -Dfile.encoding=UTF-8" ^
    %ROBOT_IMAGE% robot reason --input %COMPONENTSDIR%/all_NotReasoned.owl --output %COMPONENTSDIR%/all.owl
...
docker run --rm -v %cd%:/work -w /work -e "ROBOT_JAVA_ARGS=-Xmx8G -Dfile.encoding=UTF-8" ^
    %ROBOT_IMAGE% robot explain --reasoner hermit --input %COMPONENTSDIR%/all.owl -M inconsistency --explanation %VALIDATIONSDIR%/inconsistency_hermit.md
