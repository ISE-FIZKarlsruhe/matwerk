...
docker run --rm -v %cd%:/work -w /work -e "ROBOT_JAVA_ARGS=-Xmx8G -Dfile.encoding=UTF-8" ^
      %ROBOT_IMAGE% robot reason --reasoner hermit --input %COMPONENTSDIR%/all_NotReasoned.owl --axiom-generators "SubClass EquivalentClass DisjointClasses  ClassAssertion" --include-indirect true reduce --output %COMPONENTSDIR%/all.ttl
