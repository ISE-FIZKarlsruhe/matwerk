...
The `city` tab of the MatWerk workbook currently holds **38 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/city/pattern.ttl
height: 560px
reasoning: false
```
...
| Value | `A` | `nfdi:NFDI_0001007` | has value |

## How to read it

* The **entity** is typed `nfdi:NFDI_0000106` (city) and carries only its identity, its label, and links.
...
```sparql
SELECT ?x ?label WHERE { ?x a <https://nfdi.fiz-karlsruhe.de/ontology/NFDI_0000106> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`city` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=1469482382).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. In which city and country is a given organisation or facility located?
...
| **Also Known As** | the `city` template tab of the NFDI-MatWerk workbook |
| **Intent** | To represent a city in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | In which city and country is a given organisation or facility located? |
| **Solution description** | An entity individual typed `nfdi:NFDI_0000106` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “Dusseldorf” is a city, with denoted by “Dusseldorf_”. |
...
| **Extracted From** | the `city` tab of the NFDI-MatWerk curation workbook (38 rows at generation time) |
