...
The `sparql_endpoints` tab of the MatWerk workbook currently holds **23 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
|---|---|---|
| sparql endpoint | `nfdi:NFDI_0001095` | 23 |
...
| **Intent** | To represent a dataset in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
...
| **Extracted From** | the `sparql_endpoints` tab of the NFDI-MatWerk curation workbook (23 rows at generation time) |
