...
The `value-nfdi-matwerk-consortium` tab of the MatWerk workbook currently holds **38 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
| temporal region | `I` | `obo:BFO_0000199` | occupies temporal region |
| denoted by | `I` | `obo:IAO_0000235` | denoted by |
| has file format | `I` | `nfdi:NFDI_0000132` |  |
| has acronym | `A` | `nfdi:NFDI_0010015` | has acronym |
| has url | `AT` | `nfdi:NFDI_0001008` | has url |
| is quality measured as | `I` | `obo:IAO_0000417` | is quality measured as |
| has measurement datum | `I` | `mwo:MWO_0001116` | has measurement datum |
| is about | `I` | `obo:IAO_0000136` | is about |

## How to read it

* The **entity** is typed `mwo:MWO_0001022` (nfdi matwerk consortium) and carries only its identity, its label, and links.
...
```sparql
SELECT ?x ?label WHERE { ?x a <http://purls.helmholtz-metadaten.de/mwo/MWO_0001022> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```
...
| **Intent** | To represent a nfdi matwerk consortium in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | — |
| **Solution description** | An entity individual typed `mwo:MWO_0001022` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “NFDI-MatWerk” is a nfdi matwerk consortium. |
...
| **Extracted From** | the `value-nfdi-matwerk-consortium` tab of the NFDI-MatWerk curation workbook (38 rows at generation time) |
