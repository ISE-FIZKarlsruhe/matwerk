...
The `people` tab of the MatWerk workbook currently holds **60 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/people/pattern.ttl
height: 560px
reasoning: false
```
...
| First name | `I` | `obo:IAO_0000235` | denoted by |
| Last name | `I` | `obo:IAO_0000235` | denoted by |
| E-Mail* | `I` | `obo:IAO_0000235` | denoted by |
| Academic discipline | `I` | `nfdi:NFDI_0000211` |  |
| ORCID ID | `I` | `nfdi:NFDI_0001006` |  |
| participates in | `I` | `obo:RO_0000056` | participates in |

## How to read it

* The **entity** is typed `nfdi:NFDI_0000004` (person) and carries only its identity, its label, and links.
...
|---|---|---|
| given name | `obo:IAO_0020016` | 60 |
| family name | `obo:IAO_0020017` | 60 |
| project | `nfdi:NFDI_0000002` | 60 |
| ORCID identifier | `obo:IAO_0000708` | 47 |
...
```sparql
SELECT ?x ?label WHERE { ?x a <https://nfdi.fiz-karlsruhe.de/ontology/NFDI_0000004> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`people` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=1666156492).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which researchers are involved in NFDI-MatWerk, and in which task area?
2. What is the ORCID iD of a given researcher, and which organisation are they affiliated with?
3. Which academic disciplines does a researcher work in?
4. Which datasets, software or publications is a given researcher a creator of?
...
| **Also Known As** | the `people` template tab of the NFDI-MatWerk workbook |
| **Intent** | To represent a person in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | Which researchers are involved in NFDI-MatWerk, and in which task area?; What is the ORCID iD of a given researcher, and which organisation are they affiliated with?; Which academic disciplines does a researcher work in?; Which datasets, software or publications is a given researcher a creator of? |
| **Solution description** | An entity individual typed `nfdi:NFDI_0000004` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “Dr. Sarath Menon” is a person, with title “Dr.”; first name “Sarath”; last name “Menon”. |
...
| **Extracted From** | the `people` tab of the NFDI-MatWerk curation workbook (60 rows at generation time) |
| **Reengineered From** | the ROBOT template directives in row 2 of that tab |
| **Has Components** | given name, family name, project, ORCID identifier |
...
| **Axioms / logical axioms** | 20 / 6 |
| **Classes / individuals** | 6 / 7 |
