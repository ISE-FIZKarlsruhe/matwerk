...
The `role` tab of the MatWerk workbook currently holds **650 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/role/pattern.ttl
height: 560px
reasoning: true
```
...
| role email | `I` | `obo:IAO_0000235` | denoted by |
| role website | `I` | `obo:IAO_0000235` | denoted by |
| denoted by written name | `I` | `obo:IAO_0000235` | denoted by |
| role of | `I` | `obo:RO_0000081` | role of |

## How to read it

* The **entity** is typed `nfdi:NFDI_0000114` (contact point role) and carries only its identity, its label, and links.
...
|---|---|---|
| email address | `obo:IAO_0000429` | 76 |
| agent | `nfdi:NFDI_0000102` | 34 |
...
```sparql
SELECT ?x ?label WHERE { ?x a <https://nfdi.fiz-karlsruhe.de/ontology/NFDI_0000114> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`role` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=1425127117).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which role does a given person or organisation bear, and in what context?
2. Which contact point is responsible for a given resource?
...
| **Also Known As** | the `role` template tab of the NFDI-MatWerk workbook |
| **Intent** | To represent a contact point role in the MSE Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | Which role does a given person or organisation bear, and in what context?; Which contact point is responsible for a given resource? |
| **Solution description** | An entity individual typed `nfdi:NFDI_0000114` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “ContactPointRole 9” is a contact point role, with role email “support@kitware.com”; role website “url 384”; role of “ContactPoint 9”. |
...
| **Extracted From** | the `role` tab of the NFDI-MatWerk curation workbook (650 rows at generation time) |
| **Reengineered From** | the ROBOT template directives in row 2 of that tab |
| **Has Components** | email address, agent |
