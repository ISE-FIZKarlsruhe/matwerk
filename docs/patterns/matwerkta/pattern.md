...
The `matwerkta` tab of the MatWerk workbook currently holds **11 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/matwerkta/pattern.ttl
height: 560px
reasoning: true
```
...
| Main goals | `I` | `nfdi:NFDI_0000226` | is subject of |
| Description | `I` | `obo:IAO_0000235` | denoted by |
| participates in process | `I` | `obo:RO_0000056` | participates in |
| Measures | `I` | `obo:IAO_0000235` | denoted by |
| Work Packages | `I` | `obo:IAO_0000235` | denoted by |
| has part | `I` | `obo:BFO_0000051` | has part |

## How to read it

* The **entity** is typed `obo:IAO_0000005` (objective specification) and carries only its identity, its label, and links.
...
|---|---|---|
| work package textual entity | `nfdi:NFDI_0001035` | 5 |
| process | `obo:BFO_0000015` | 5 |
...
```sparql
SELECT ?x ?label WHERE { ?x a <http://purl.obolibrary.org/obo/IAO_0000005> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`matwerkta` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=1489640604).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which task areas make up NFDI-MatWerk, and what is each responsible for?
2. Which researchers and organisations contribute to a given task area?
3. Which infrastructure use cases and participant projects does a task area cover?
...
| **Also Known As** | goal specification; goal specification; goal specification |
| **Intent** | To represent a objective specification in the MSE Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | Which task areas make up NFDI-MatWerk, and what is each responsible for?; Which researchers and organisations contribute to a given task area?; Which infrastructure use cases and participant projects does a task area cover? |
| **Solution description** | An entity individual typed `obo:IAO_0000005` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “TA-SD” is a objective specification, with name “Strategy Development”; main goals “Support the digital transformation of materials science through a community-driven process; Monitoring, analysis, feedback of the current state in the community”; description “This TA is responsible for managing the development of a digital strategy for our consortium and the MSE community.”. |
...
| **Extracted From** | the `matwerkta` tab of the NFDI-MatWerk curation workbook (11 rows at generation time) |
| **Reengineered From** | the ROBOT template directives in row 2 of that tab |
| **Has Components** | work package textual entity, process |
...
| **Axioms / logical axioms** | 49 / 11 |
| **Classes / individuals** | 5 / 6 |
