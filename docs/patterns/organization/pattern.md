...
The `organization` tab of the MatWerk workbook currently holds **56 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/organization/pattern.ttl
height: 560px
reasoning: false
```
...
| Institution name | `I` | `obo:IAO_0000235` | denoted by |
| DE label | `A` | `rdfs:label` | label |
| Acronym | `A` | `nfdi:NFDI_0010015` | has acronym |
| City | `I` | `obo:RO_0001025` | located in |
| Zip Code | `I` | `obo:IAO_0000235` | denoted by |
| country | `I` | `obo:RO_0001025` | located in |
| Institute Website | `I` | `obo:IAO_0000235` | denoted by |
| Parent organization | `I` | `nfdi:NFDI_0000182` |  |
| ROR ID | `I` | `nfdi:NFDI_0001006` |  |

## How to read it

* The **entity** is typed `obo:OBI_0000245` (organization) and carries only its identity, its label, and links.
...
|---|---|---|
| institutional identification | `obo:IAO_0000303` | 56 |
| country | `nfdi:NFDI_0000119` | 43 |
| ROR identifier | `nfdi:NFDI_0001013` | 28 |
| zone improvement plan code | `nfdi:NFDI_0001042` | 27 |
...
```sparql
SELECT ?x ?label WHERE { ?x a <http://purl.obolibrary.org/obo/OBI_0000245> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`organization` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=447157523).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which organisations participate in NFDI-MatWerk?
2. What is an organisation's ROR identifier, and in which city is it located?
3. Which organisation is the parent of a given institute?
4. Which researchers, instruments and facilities belong to a given organisation?
...
| **Also Known As** | the `organization` template tab of the NFDI-MatWerk workbook |
| **Intent** | To represent a organization in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | Which organisations participate in NFDI-MatWerk?; What is an organisation's ROR identifier, and in which city is it located?; Which organisation is the parent of a given institute?; Which researchers, instruments and facilities belong to a given organisation? |
| **Solution description** | An entity individual typed `obo:OBI_0000245` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “Max Planck Institute for Iron Research” is a organization, with institution name “Max Planck Institute for Iron Research_name_”; city “Dusseldorf”; zip code “40237”. |
...
| **Extracted From** | the `organization` tab of the NFDI-MatWerk curation workbook (56 rows at generation time) |
| **Reengineered From** | the ROBOT template directives in row 2 of that tab |
| **Has Components** | institutional identification, country, ROR identifier, zone improvement plan code |
| **Specialization Of** | the entity-and-value-node pattern shared by every tab ([overview](../index.md)) |
| **Related CPs** | [city](../city/pattern.md) |
...
| **Axioms / logical axioms** | 25 / 8 |
| **Classes / individuals** | 7 / 8 |
