...
The `agent` tab of the MatWerk workbook currently holds **591 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/agent/pattern.ttl
height: 560px
reasoning: false
```
...
* The **entity** is typed `nfdi:NFDI_0000102` (agent) and carries only its identity, its label, and links.
...
```sparql
SELECT ?x ?label WHERE { ?x a <https://nfdi.fiz-karlsruhe.de/ontology/NFDI_0000102> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`agent` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=2077140060).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which agents (people, organisations, consortia) does the knowledge graph know?
2. Which agent is responsible for a given resource?
...
| **Also Known As** | the `agent` template tab of the NFDI-MatWerk workbook |
| **Intent** | To represent a agent in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | Which agents (people, organisations, consortia) does the knowledge graph know?; Which agent is responsible for a given resource? |
| **Solution description** | An entity individual typed `nfdi:NFDI_0000102` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “ContactPoint 1” is a agent, with denoted by written name “WrittenName 1”. |
...
| **Extracted From** | the `agent` tab of the NFDI-MatWerk curation workbook (591 rows at generation time) |
