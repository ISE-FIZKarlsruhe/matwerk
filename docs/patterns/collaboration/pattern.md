...
```ontoink
source: patterns/collaboration/pattern.ttl
height: 560px
reasoning: true
```
...
|---|---|---|---|
| Collabration title | `I` | `obo:IAO_0000235` | denoted by |
| Collabration partner | `I` | `obo:RO_0000057` | has participant |
| Consortium partner | `I` | `obo:RO_0000057` | has participant |
| Target | `I` | `obo:RO_0000057` | has participant |
| Status | `I` | `obo:RO_0000059` | concretizes |
| occupies temporal region | `I` | `obo:BFO_0000199` | occupies temporal region |
| Comment | `I` | `obo:IAO_0000235` | denoted by |

## How to read it

* The **entity** is typed `mwo:MWO_0001007` (joint efforting) and carries only its identity, its label, and links.
...
|---|---|---|
| consortium | `nfdi:NFDI_0000006` | 22 |
| comment textual entity | `nfdi:NFDI_0001036` | 2 |
...
```sparql
SELECT ?x ?label WHERE { ?x a <http://purls.helmholtz-metadaten.de/mwo/MWO_0001007> ; <http://purl.obolibrary.org/obo/IAO_0000235> ?d . ?d <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`collaboration` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=266847052).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which international collaborations is NFDI-MatWerk part of?
2. Which organisations and researchers take part in a given collaboration?
...
| **Also Known As** | the `collaboration` template tab of the NFDI-MatWerk workbook |
| **Intent** | To represent a joint efforting in the MSE Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | Which international collaborations is NFDI-MatWerk part of?; Which organisations and researchers take part in a given collaboration? |
| **Solution description** | An entity individual typed `mwo:MWO_0001007` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “DFT code integration__name__” is a joint efforting, with collabration title “DFT code integration__name__”; collabration partner “University College London (UCL)|National Institute for Materials Science (NIMS)”; consortium partner “Federal Institute For Materials Research and Testing|Max Planck Institute for Iron Research”. |
...
| **Extracted From** | the `collaboration` tab of the NFDI-MatWerk curation workbook (25 rows at generation time) |
| **Reengineered From** | the ROBOT template directives in row 2 of that tab |
| **Has Components** | consortium, comment textual entity |
| **Specialization Of** | the entity-and-value-node pattern shared by every tab ([overview](../index.md)) |
| **Related CPs** | [organization](../organization/pattern.md), [temporal](../temporal/pattern.md) |
