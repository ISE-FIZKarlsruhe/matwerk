...
```ontoink
source: patterns/materials/pattern.ttl
height: 560px
reasoning: true
```
...
```sparql
SELECT ?x ?label WHERE { ?x a <http://purls.helmholtz-metadaten.de/mwo/MWO_0001099> ; <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`materials` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=497166822).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which materials are described in the knowledge graph?
2. Which datasets, publications or instruments concern a given material?
...
| **Intent** | To represent a material designation in the MSE Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
...
| **Extracted From** | the `materials` tab of the NFDI-MatWerk curation workbook (25 rows at generation time) |
