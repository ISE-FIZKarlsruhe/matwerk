...
The `metadata` tab of the MatWerk workbook currently holds **20 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/metadata/pattern.ttl
height: 560px
reasoning: false
```
...
| Documentation link | `I` | `obo:IAO_0000235` | denoted by |
...
| Related Publication DOI | `I` | `obo:IAO_0000235` | denoted by |
| Related Project collabration | `I` | `nfdi:NFDI_0000194` |  |

## How to read it

* The **entity** is typed `nfdi:NFDI_0001054` (obsolete metadata specification) and carries only its identity, its label, and links.
...
The rows behind it are curated in the [`metadata` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=278046522).
...
| **Intent** | To represent a obsolete metadata specification in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
...
| **Extracted From** | the `metadata` tab of the NFDI-MatWerk curation workbook (20 rows at generation time) |
