...
The `ontologies` tab of the MatWerk workbook currently holds **109 rows**. Every row follows the same shape, and that shape is the pattern below: a **entity** instance carrying the row's identity, denoted by **value node** nodes that hold its values, and object properties pointing at entities in other tabs.
...
```ontoink
source: patterns/ontologies/pattern.ttl
height: 560px
reasoning: false
```
...
| Link (URL / PID) to repository | `I` | `obo:IAO_0000235` | denoted by |
| Link (URL / PID) to documentation | `I` | `obo:IAO_0000235` | denoted by |
...
* The **entity** is typed `nfdi:NFDI_0000023` (ontology) and carries only its identity, its label, and links.
...
|---|---|---|
| written name | `obo:IAO_0000590` | 105 |
| source code repository | `nfdi:NFDI_0000030` | 101 |
...
```sparql
SELECT ?x ?label WHERE { ?x a <https://nfdi.fiz-karlsruhe.de/ontology/NFDI_0000023> ; <http://purl.obolibrary.org/obo/IAO_0000235> ?d . ?d <http://www.w3.org/2000/01/rdf-schema#label> ?label } LIMIT 10
```

The rows behind it are curated in the [`ontologies` tab of the workbook](https://docs.google.com/spreadsheets/d/1OyoWwcX4zUtrJilwXdtTooavELw278nQSNW2oniBBsk/edit#gid=2006199416).

## Competency questions

The questions this pattern exists to answer. Each is answerable against the published graphs.

1. Which ontologies are used or developed in NFDI-MatWerk?
2. What is the version and licence of a given ontology, and where is it published?
3. Which ontology is reused by a given metadata standard or knowledge graph?
...
| **Also Known As** | the `ontologies` template tab of the NFDI-MatWerk workbook |
| **Intent** | To represent a ontology in the MatWerk Knowledge Graph: its identity, the value nodes that describe it, and its links to the other entities the NFDI-MatWerk consortium records. |
| **Domains** | research data management; NFDI consortium structure |
| **Competency Questions** | Which ontologies are used or developed in NFDI-MatWerk?; What is the version and licence of a given ontology, and where is it published?; Which ontology is reused by a given metadata standard or knowledge graph? |
| **Solution description** | An entity individual typed `nfdi:NFDI_0000023` carries identity and label; each descriptive value is a separate *value node* individual reached by `obo:IAO_0000235` (denoted by); links to other patterns are object properties onto those entities. Identity is a minted IRI `msekg:<epoch_ms><counter>`, so re-running the pipeline is idempotent. |
...
| **Scenarios** | “Dislocation Ontology (DISO)” is a ontology, with name “Dislocation Ontology (DISO)”; link (url / pid) to repository “url 570”; link (url / pid) to documentation “url 666”. |
...
| **Extracted From** | the `ontologies` tab of the NFDI-MatWerk curation workbook (109 rows at generation time) |
| **Reengineered From** | the ROBOT template directives in row 2 of that tab |
| **Has Components** | written name, source code repository |
