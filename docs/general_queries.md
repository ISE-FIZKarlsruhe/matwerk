...
SELECT DISTINCT ?person ?label ?email ?orcid WHERE {
  ...
  OPTIONAL {
    ?person has_external_identifier: ?orcid .
  }
}
LIMIT 999
```
...
### Which organizations in the MSE-KG can be linked to Wikidata via their ROR ID, and what are their city, acronym, and country?
...
### Which people in the MSE-KG are linked to Wikidata through ORCID, and what are their employers and countries?
